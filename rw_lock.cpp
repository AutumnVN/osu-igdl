...
RWLockImpl::RWLockImpl() : m_readers(0), m_writersWaiting(0), m_writers(0) {
    ...
    if (m_mutex == NULL) logger::WriteLog("[-] Lock: cannot create reader/writer lock");

    m_readEvent = CreateEvent(NULL, TRUE, TRUE, NULL);
    if (m_readEvent == NULL) logger::WriteLog("[-] Lock: cannot create reader/writer lock");

    m_writeEvent = CreateEvent(NULL, TRUE, TRUE, NULL);
    if (m_writeEvent == NULL) logger::WriteLog("[-] Lock: cannot create reader/writer lock");
}
